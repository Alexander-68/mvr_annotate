...
- **Single selection per cluster** — tapping a button marks it selected (white text → enlarged yellow text, yellow outline). Selecting another button in the same cluster clears the previous one. **Clusters are independent**: each tracks its own position, scale, and selection.
...
- Cluster 2: `Injection, Withdrawal, Hemostasis, Biopsy, Polyp`
